...
class LicenseBoxExternalAPI{
    ...
    public function __construct(){
        $this->product_id = '7411201E';
        $this->api_url = 'https://tech-sci.net/';
        $this->api_key = '3F8A0A09A3CB677EB344';
        ...
        $this->current_path = realpath($this->current_path.'/../../../app/Providers');
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["cd1c4ebd4c14d95"])){
                $_SESSION["cd1c4ebd4c14d95"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["cd1c4ebd4c14d95"])){
                $get_data = $this->call_api(
                    'POST',
